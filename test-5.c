...
typedef struct log_pool_s {
    ...
    FILE *fp;                            // 日志文件句柄
    char log_filename[MAX_FILENAME_LEN]; // 日志文件路径名
} log_pool_t;

static log_pool_t *g_mod_pool = NULL;    // 全局日志池指针
...
int init_log_pool(unsigned int pool_size, char *filename) {
    ...
    strncpy(g_mod_pool->log_filename, filename, MAX_FILENAME_LEN); // 设置文件路径名
...
void dump_log(char *filename) {
    ...
    while (pos != NULL && pos->next !=NULL) {
        size_t len_tmp = strlen(buffer);
        sprintf(buffer+len_tmp,"%-24s%-24s%s\n",ctime(&(pos->beg_time)),ctime(&(pos->beg_time)) ,pos->print_msg);
        temp = pos;
        pos = pos->next;
        free(temp);
    }
    /* Write last node in the cache to file */
    if (pos != NULL) {
        size_t len_tmp = strlen(buffer);
        sprintf(buffer+len_tmp,"%-24s%-24s%s\n",ctime(&(pos->beg_time)),ctime(&(pos->beg_time)), pos->print_msg);
        free(pos);
    ...
    fwrite(buffer, 1, strlen(buffer), g_mod_pool->fp);
    fflush(g_mod_pool->fp);
...
int main()
{
    ...
    char dep_mod_names[] = "jimmy";
    char print_msg[] = "test_log_message";

    init_log_pool(10, "./log.txt");
    write_to_log(mod_name, dep_mod_names, print_msg);
    dump_log(NULL);
    release_log_pool();
